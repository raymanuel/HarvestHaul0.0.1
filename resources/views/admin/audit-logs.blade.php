...
<div class="w-full max-w-7xl mx-auto">
    ...
    <div class="bg-white dark:bg-slate-800 border border-slate-200/70 dark:border-slate-700/80 rounded-2xl shadow-sm overflow-hidden">
            ...
            <table class="w-full text-sm text-left" style="min-width: 700px;">
                <thead>
                    <tr class="border-b border-slate-100 dark:border-slate-700/60 bg-slate-50/50 dark:bg-slate-900/40">
                        ...
                        <th class="px-6 py-4 text-[10px] font-extrabold text-slate-500 dark:text-slate-500 uppercase tracking-widest">Performed By</th>
                        <th class="px-6 py-4 text-[10px] font-extrabold text-slate-500 dark:text-slate-500 uppercase tracking-widest">Target</th>
                        <th class="px-6 py-4 text-[10px] font-extrabold text-slate-500 dark:text-slate-500 uppercase tracking-widest">Notes</th>
                        <th class="px-6 py-4 text-[10px] font-extrabold text-slate-500 dark:text-slate-500 uppercase tracking-widest">Date & Time</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-50 dark:divide-slate-700/40">
                    ...
                    <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-900/40 transition">
                        <td class="px-6 py-4">
                            <span class="bg-sky-50 dark:bg-sky-950/20 text-sky-700 dark:text-sky-400 text-[10px] font-bold px-2.5 py-1 rounded-lg uppercase tracking-wide">
                        ...
                        <td class="px-6 py-4 text-slate-400 dark:text-slate-550 text-xs font-semibold whitespace-nowrap">{{ $log->created_at->format('M d, Y h:i A') }}</td>
                    </tr>
                    @empty
                    <tr>
        ...
        @if($logs->hasPages())
            <div class="px-6 py-4 border-t border-slate-100 dark:border-slate-700/60 bg-slate-50/30 dark:bg-slate-900/20">
                {{ $logs->links() }}
            </div>
        @endif
    </div>
