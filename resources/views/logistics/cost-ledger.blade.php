    ...
    <div class="w-full max-w-4xl mx-auto pb-12">
        ...
        <div class="bg-white dark:bg-slate-800 border border-slate-200/70 dark:border-slate-700/80 rounded-2xl shadow-sm overflow-hidden mb-6">
            <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-700/60 flex items-center justify-between">
                ...
                @if($totalPrice > 0)
                    <span class="text-[10px] font-bold text-slate-400 bg-slate-50 dark:bg-slate-900/50 px-2.5 py-1 rounded-md border border-slate-200/60 dark:border-slate-700">
                        Weight &times; distance split
                    </span>
                @else
                    <span class="text-[10px] font-bold text-[var(--color-warning-text)] bg-[var(--color-warning-bg)] px-2.5 py-1 rounded-md border border-[var(--color-warning-border)]">
                                        ...
                                        @if(Auth::id() === $entry['farmer_id'])
                                            @if($entry['payment_status'] !== 'paid')
                                                <form action="{{ route('pooling.cost-ledger.upload-receipt', [$poolingJob->id, $entry['harvest_id']]) }}" method="POST" enctype="multipart/form-data" class="flex items-center justify-end">
                                                    @csrf
                                                    <label class="cursor-pointer bg-slate-100 hover:bg-slate-200 dark:bg-slate-700 dark:hover:bg-slate-600 border border-slate-300 dark:border-slate-600 rounded-xl px-2.5 py-1 text-xs text-slate-700 dark:text-slate-200 font-semibold transition inline-flex items-center gap-1.5 shadow-sm">
                                                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                            ...
                                            @if($entry['payment_status'] === 'submitted')
                                                <form action="{{ route('pooling.cost-ledger.mark-paid', [$poolingJob->id, $entry['harvest_id']]) }}" method="POST" class="flex items-center justify-end gap-2" onsubmit="event.preventDefault(); swalConfirm(this, {title:'Mark as Paid?', text:'Confirm this farmer has been paid this amount?', icon:'question', confirmText:'Yes, mark paid', cancelText:'Cancel', confirmColor:'#16283C'});">
                                                    ...
                                                    <input id="amount-paid-{{ $entry['harvest_id'] }}" type="number" name="amount_paid" min="0.01" step="0.01" placeholder="₱ amount"
                                                           class="w-28 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 px-2 py-1.5 text-xs text-slate-700 dark:text-slate-200 focus:outline-none focus:ring-2 focus:ring-brand">
                                                    <button type="submit" class="bg-brand hover:bg-brand-dark text-white dark:bg-[#D7BC7A] dark:hover:bg-[#BFA05A] dark:text-[#17202B] rounded-xl px-3 py-1.5 text-xs font-bold shadow-sm transition hover:shadow-md inline-flex items-center gap-1">
                                                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
