...
<div class="w-full max-w-7xl mx-auto">
    ...
    <header class="mb-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <span class="text-[10px] font-bold uppercase tracking-widest text-emerald-700 bg-emerald-50 px-3 py-1.5 rounded-lg border border-emerald-500/10 self-start">{{ $categories->count() }} Categories</span>
        </div>
    </header>

    {{-- Flash Messages --}}
    @if (session('success'))
        <div class="mb-6 bg-emerald-50 border border-emerald-200 text-emerald-700 rounded-xl px-5 py-4 text-sm font-semibold flex items-center gap-2">
        ...
        <div class="bg-white border border-slate-200/70 rounded-2xl shadow-sm p-6">
            ...
            <form method="POST" action="{{ route('admin.crops.categories.store') }}" class="flex flex-col gap-3">
                ...
                <input type="text" name="name" placeholder="Category name (e.g. Fruits)"
                    class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 bg-slate-50/50 text-slate-800 placeholder-slate-400 transition"
                    required />
        ...
        <div class="bg-white border border-slate-200/70 rounded-2xl shadow-sm p-6">
            ...
            <form method="POST" action="{{ route('admin.crops.store') }}" class="flex flex-col gap-3">
                ...
                <select name="crop_category_id"
                    class="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 bg-slate-50/50 text-slate-800 transition"
                    required>
        ...
        <div class="bg-white border border-slate-200/70 rounded-2xl shadow-sm p-6">
            ...
            <form method="POST" action="{{ route('admin.crops.varieties.store') }}" class="flex flex-col gap-3">
                ...
                <input type="number" name="price_per_kg" placeholder="Price/kg (₱)" step="0.01" min="0"
                    ...
                    required />
                <button type="submit"
                    class="w-full bg-emerald-600 text-white text-xs font-bold px-5 py-2.5 rounded-xl hover:bg-emerald-700 transition shadow-sm">
                    Add Variety
                </button>
            </form>
        </div>
    ...
    <div class="mb-6 bg-white border border-slate-200/70 rounded-2xl shadow-sm overflow-hidden">
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-slate-100 bg-slate-50/50">
            <div class="flex items-center gap-3">
                <div class="w-8 h-8 rounded-lg bg-gradient-to-tr from-emerald-100 to-emerald-50 border border-emerald-200/50 flex items-center justify-center text-[10px] font-extrabold text-emerald-700 uppercase">{{ substr($category->name, 0, 2) }}</div>
                <div>
            ...
            <div class="flex gap-2">
                <button onclick="toggleModal('edit-category-{{ $category->id }}')"
                    class="text-[10px] text-slate-500 hover:text-slate-800 border border-slate-200 rounded-lg px-3 py-1.5 font-bold uppercase tracking-wide transition hover:bg-slate-100">
                    Edit
                </button>
        ...
        <div class="px-6 py-5 border-b border-slate-50 last:border-0">
            ...
            <div class="flex items-center justify-between mb-4">
                <div class="flex items-center gap-3">
                    ...
                    <span class="text-[10px] font-bold px-2 py-0.5 rounded-lg uppercase tracking-wide {{ $crop->is_active ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-400' }}">
                        {{ $crop->is_active ? 'Active' : 'Inactive' }}
                    </span>
                </div>
                <div class="flex gap-2">
                    <button onclick="toggleModal('edit-crop-{{ $crop->id }}')"
                        class="text-[10px] text-slate-500 hover:text-slate-800 border border-slate-200 rounded-lg px-3 py-1 font-bold uppercase tracking-wide transition hover:bg-slate-100">
                        Edit
                    </button>
                    ...
                    <tbody>
                        ...
                        <tr class="border-b border-slate-50 last:border-0 hover:bg-white/50 transition">
                            ...
                            <td class="px-4 py-2.5">
                                <span class="text-[10px] font-bold px-2 py-0.5 rounded-lg uppercase tracking-wide {{ $variety->is_active ? 'bg-emerald-50 text-emerald-600' : 'bg-slate-100 text-slate-400' }}">
