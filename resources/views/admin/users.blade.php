...
<div class="w-full max-w-7xl mx-auto">
    ...
    <x-data-table>
        ...
        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-900/40 transition">
            ...
            <td class="px-4 py-3">
                <span class="bg-slate-100 dark:bg-slate-900/50 text-slate-700 dark:text-slate-300 text-[10px] font-bold px-2.5 py-1 rounded-md uppercase tracking-wide">
            ...
            <td class="px-4 py-3 text-right">
                <div class="flex items-center justify-end gap-3.5">
                    @if($user->role !== 'admin')
                        ...
                        class="inline-flex items-center justify-center w-8 h-8 rounded-md bg-[#16283C]/10 text-[#16283C] hover:bg-[#16283C]/15 dark:bg-[#16283C]/10 dark:hover:bg-[#16283C]/15 dark:text-[#D7BC7A] transition"
                        title="Edit User">
                        ...
                        @if($user->status === 'active')
                            @if($user->role === 'farmer')
                                <button type="button" onclick="checkAndArchive({{ $user->id }}, '{{ addslashes($user->name) }}')" class="inline-flex items-center justify-center w-8 h-8 rounded-xl bg-amber-50 text-amber-700 hover:bg-amber-100 dark:bg-amber-950/20 dark:hover:bg-amber-950/40 dark:text-amber-400 transition" title="Archive User">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        ...
                        @else
                            <form method="POST" action="{{ route('admin.users.status', $user->id) }}" class="inline" id="reactivate-form-{{ $user->id }}">
                                @csrf
                                <button type="button" onclick="swalConfirm(this.closest('form'), {title: 'Reactivate User?', text: 'Reactivate {{ addslashes($user->name) }}?', confirmText: 'Yes, reactivate', icon: 'question', confirmColor: '#16283C'})" class="inline-flex items-center justify-center w-8 h-8 rounded-xl bg-[#16283C]/10 text-[#16283C] hover:bg-[#16283C]/15 dark:bg-[#16283C]/10 dark:hover:bg-[#16283C]/15 dark:text-[#D7BC7A] transition" title="Reactivate User">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
