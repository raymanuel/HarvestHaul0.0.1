...
<div class="w-full max-w-7xl mx-auto pb-12">
    ...
    @php
        ...
        $accentBg = 'bg-[#16283C] hover:bg-[#0E1620] dark:bg-[#D7BC7A] dark:hover:bg-[#BFA05A] dark:text-[#17202B]';
        ...
        $currentRate = $haulIntent->hauling_rate_php_per_kg ?? ($haulIntent->offer_rate_php_per_kg ?? $haulIntent->counter_rate_php_per_kg);
        $haulKg = $haulIntent->haulRequest->harvest?->quantity_kg;
    @endphp
        ...
        <header class="mb-6 pt-8">
            <div class="flex items-center gap-2 mb-2">
                <a href="{{ $backRoute }}" class="text-xs font-bold {{ $accentText }} hover:underline flex items-center gap-1">
                    ← Back to {{ $isFarmer ? 'Haul Requests' : 'Haul Deals' }}
                </a>
                <span class="text-xs font-bold {{ $accentText }} opacity-60">· Deal #{{ $haulIntent->id }}</span>
            </div>
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div>
                    <span id="deal-status-badge" class="text-[10px] font-extrabold uppercase tracking-widest px-3 py-1.5 rounded-md border shadow-sm
        ...
        <div class="flex flex-col lg:flex-row bg-white dark:bg-slate-800 border border-slate-200/60 dark:border-slate-700/60 rounded-3xl overflow-hidden shadow-sm h-[min(820px,88vh)]">
            ...
            <div class="flex flex-col flex-1 min-w-0 min-h-0">
                <div class="px-6 py-4 bg-slate-50/50 dark:bg-slate-900/40 border-b border-slate-150 dark:border-slate-700/60 flex items-center justify-between shrink-0">
                    ...
                    <span class="text-[10px] font-bold font-mono text-slate-500 dark:text-slate-400">Private haul chat</span>
                </div>
                            ...
                            <div class="flex {{ $isMine ? 'justify-end' : 'justify-start' }}">
                                <div class="max-w-[70%] flex flex-col {{ $isMine ? 'items-end' : 'items-start' }}">
                                    ...
                                    <div class="px-4 py-3 rounded-2xl text-xs leading-relaxed shadow-sm font-medium
                                        @if($isMine) bg-[#16283C] dark:bg-[#D7BC7A] dark:text-[#17202B] text-white rounded-br-none
            ...
            <aside class="w-full lg:w-[320px] shrink-0 min-h-0 max-h-[40%] lg:max-h-none p-6 overflow-y-auto border-t lg:border-t-0 lg:border-l border-slate-200/60 dark:border-slate-700/60">
                ...
                <div class="border-t border-slate-200/60 dark:border-slate-700/60 my-4 pt-1"></div>
                ...
                <h3 class="text-sm font-extrabold text-slate-800 dark:text-white heading-font mb-4 uppercase tracking-wider">Proposed Hauling Rate</h3>
                    <div class="mb-4 divide-y divide-slate-100 dark:divide-slate-700/40">
                        <div class="flex justify-between items-center gap-3 py-2.5">
                            <span class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Current Rate</span>
                            <span id="current-rate" class="text-sm font-bold text-slate-800 dark:text-white font-mono">{{ $haulIntent->hauling_rate_php_per_kg ? '₱'.number_format((float) $haulIntent->hauling_rate_php_per_kg, 2).' /kg' : '—' }}</span>
                        </div>
                        <div class="flex justify-between items-center gap-3 py-2.5">
                            <span class="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Haul Cost</span>
                            <span id="haul-total" class="text-sm font-black text-slate-800 dark:text-white font-mono">{{ ($currentRate !== null && $haulKg) ? '₱'.number_format((float) $currentRate * (float) $haulKg, 2) : '—' }}</span>
                        </div>
                        <div class="py-2.5">
                            <p class="text-[10px] text-slate-500 dark:text-slate-400 leading-relaxed">Logistics offer: <span id="offer-rate" class="font-bold">{{ $haulIntent->offer_rate_php_per_kg ? '₱'.number_format((float) $haulIntent->offer_rate_php_per_kg, 2) : '—' }}</span> · Farmer counter: <span id="counter-rate" class="font-bold">{{ $haulIntent->counter_rate_php_per_kg ? '₱'.number_format((float) $haulIntent->counter_rate_php_per_kg, 2) : '—' }}</span></p>
                        </div>
                        @if(!empty($rateReference))
                            <div class="py-2.5">
                                <p class="text-[10px] text-slate-500 dark:text-slate-400 leading-relaxed">Fair road-cost reference: <span class="font-bold">~₱{{ number_format($rateReference, 2) }}/kg</span> — straight-line farm→market estimate; hilly routes cost more.</p>
                            </div>
                        @endif
                    </div>
...
<script>
    ...
    var quantityKg = {{ (float) ($haulIntent->haulRequest->harvest?->quantity_kg ?? 0) }};
    ...
    function updateUI(data) {
        ...
        var haulTotal = document.getElementById('haul-total');
        if (haulTotal && rate !== undefined && rate !== null && rate && quantityKg > 0) {
            haulTotal.textContent = '₱' + (parseFloat(rate) * quantityKg).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
        }
        if (data.status) {
        ...
        if (rate && data.status !== 'accepted' && data.status !== 'declined'
            ...
            if (rateForm) {
                rateForm.insertAdjacentHTML('afterend',
                    ...
                    '<button type="submit" id="agree-btn" class="w-full py-3 bg-[#16283C] hover:bg-[#0E1620] dark:bg-[#D7BC7A] dark:hover:bg-[#BFA05A] dark:text-[#17202B] text-white font-bold rounded-xl text-xs transition duration-200 shadow-sm cursor-pointer">' +
                    (isFarmer ? 'Agree & Book This Rate' : 'Agree to This Rate') +
                    '</button></form>');
            }
        }
