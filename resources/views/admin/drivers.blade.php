                ...
                <div class="flex items-center gap-2">
                    @if($driver->driverProfile?->id_photo_path && !$driver->driverProfile?->identity_verified)
                        <form method="POST" action="{{ route('admin.drivers.verify-identity', $driver->id) }}" class="inline">
                            @csrf
                            <button type="button" onclick="swalConfirm(this.closest('form'), {title: 'Verify Identity?', text: 'Verify identity for {{ addslashes($driver->name) }}?', confirmText: 'Yes, verify', icon: 'success', confirmColor: '#16283C'})" class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-[#16283C]/10 text-[#16283C] hover:bg-[#16283C]/15 dark:bg-[#16283C]/10 dark:hover:bg-[#16283C]/15 dark:text-[#D7BC7A] transition" title="Verify Identity">
                    ...
                    <form method="POST" action="{{ route('admin.users.status', $driver->id) }}">
                        ...
                        @else
                            <button type="button" onclick="swalConfirm(this.closest('form'), {title: 'Reactivate Driver?', text: 'Reactivate {{ addslashes($driver->name) }}?', confirmText: 'Yes, reactivate', icon: 'question', confirmColor: '#16283C'})" class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-[#16283C]/10 text-[#16283C] hover:bg-[#16283C]/15 dark:bg-[#16283C]/10 dark:hover:bg-[#16283C]/15 dark:text-[#D7BC7A] transition" title="Reactivate Driver">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
