                        ...
                        <tbody class="divide-y divide-slate-100 dark:divide-slate-700/60">
                            @foreach($harvests as $harvest)
                                ...
                                <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition">
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="text-[10px] font-extrabold uppercase tracking-wider px-2.5 py-1 rounded-lg border
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="inline-block text-[10px] font-bold px-2.5 py-1 rounded-lg border {{ $logisticsBadgeColor }}">
                                            {{ $logisticsStatus }}
                                        </span>
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        @if($harvest->status->value === 'in_progress' && $poolingJob)
                                            <a href="{{ route('tracking.index') }}"
                                                class="inline-flex items-center gap-1 text-[10px] font-bold text-orange-700 dark:text-orange-400 bg-orange-50 dark:bg-orange-950/20 hover:bg-orange-100 dark:hover:bg-orange-950/40 px-2.5 py-1.5 rounded-lg transition cursor-pointer">
                                                Track Now
                                            </a>
                                        @elseif($harvest->status->value === 'assigned' && $poolingJob && $poolingJob->status->value === 'pending')
                                            <a href="{{ route('farmer.proposals') }}"
                                                class="inline-flex items-center gap-1 text-[10px] font-bold text-purple-700 dark:text-purple-400 bg-purple-50 dark:bg-purple-950/20 hover:bg-purple-100 dark:hover:bg-purple-950/40 px-2.5 py-1.5 rounded-lg transition cursor-pointer">
                                                Review Route Offer
                                            </a>
