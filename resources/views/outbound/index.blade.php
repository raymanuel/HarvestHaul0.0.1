<x-layout title="Customer Orders">
    ...
    <div class="w-full max-w-6xl mx-auto pb-12">
        ...
        <header class="pt-8 mb-6 border-b border-slate-200/80 dark:border-slate-700/80 pb-5">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <h1 class="text-2xl font-bold text-slate-900 dark:text-white tracking-tight heading-font">Customer Orders</h1>
        ...
        @if($orders->isEmpty())
            <div class="bg-white dark:bg-slate-800 border border-slate-200/70 dark:border-slate-700/80 rounded-2xl shadow-sm p-12 text-center">
                ...
                <p class="text-slate-800 dark:text-slate-200 font-bold text-base mb-1 heading-font">No Customer Orders Yet</p>
        ...
        @else
            <x-data-table :empty-message="'No customer orders yet.'">
                <x-slot:header>
                    ...
                    <th class="px-4 py-3 text-[10px] font-extrabold text-slate-500 dark:text-slate-500 uppercase tracking-widest">Truck</th>
                    <th class="px-4 py-3 text-[10px] font-extrabold text-slate-500 dark:text-slate-500 uppercase tracking-widest">Driver</th>
                ...
                @foreach($orders as $order)
                    ...
                    <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-900/40 transition">
                        ...
                        <td class="px-4 py-3 text-slate-500 dark:text-slate-400 text-xs font-medium">{{ $order->poolingJob?->truck?->plate_number ?? '—' }}</td>
                        <td class="px-4 py-3 text-slate-500 dark:text-slate-400 text-xs font-medium">{{ $order->poolingJob?->driver?->name ?? '—' }}</td>
