    ...
    <div class="w-full max-w-4xl mx-auto pb-12">
        ...
        <header class="pt-8 mb-6 border-b border-slate-200/80 dark:border-slate-700/80 pb-5">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <span class="text-xs font-bold uppercase tracking-wider text-harvest-dark dark:text-harvest-light bg-harvest/10 dark:bg-harvest/20 px-3 py-1.5 rounded-md border border-harvest/10 dark:border-harvest/20 inline-block mb-2">Driver Fleet</span>
                    <h1 class="text-3xl font-bold text-slate-900 dark:text-white tracking-tight heading-font">Drivers</h1>
                </div>
                <div>
                    <x-button tag="a" :href="route('logistics.drivers.create')" size="lg">
        ...
        @php
            $fleetSetupTabs = [
                ['label' => 'Drivers', 'url' => route('logistics.drivers.index'), 'active' => true],
                ['label' => 'Vehicles', 'url' => route('logistics.vehicles.index'), 'active' => false],
            ];
        @endphp
        ...
        <div class="bg-white dark:bg-slate-800 border border-slate-200/70 dark:border-slate-700/80 rounded-2xl shadow-sm p-6">
            ...
            @if($drivers->isEmpty())
                <div class="bg-slate-50 dark:bg-slate-900/40 border border-dashed border-slate-300 dark:border-slate-700/80 rounded-xl p-12 text-center">
                    ...
                    <a href="{{ route('logistics.drivers.create') }}" class="mt-5 inline-block text-xs font-bold text-[#16283C] dark:text-[#D7BC7A] hover:underline transition">
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="border-b border-slate-100 dark:border-slate-700/60">
                                <th class="pb-3 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Driver Details</th>
                                <th class="pb-3 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">License Number</th>
                                <th class="pb-3 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Identity</th>
                                <th class="pb-3 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Vehicle Profile</th>
                                ...
                                <tr>
                                    <td class="py-4 pr-3">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-xs text-slate-500 dark:text-slate-400 font-medium mt-0.5">{{ $driver->user->email }}</p>
                                                <p class="text-[10px] text-slate-500 dark:text-slate-400 font-mono mt-0.5">{{ $driver->phone }}</p>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td class="py-4 px-3">
                                        @if($driver->identity_verified)
                                            <span class="bg-[#16283C]/10 dark:bg-[#16283C]/10 text-[#16283C] dark:text-[#D7BC7A] text-[10px] font-bold px-2.5 py-1 rounded-md uppercase tracking-wide">Verified</span>
                                        @elseif($driver->id_photo_path)
                                            <span class="bg-amber-50 dark:bg-amber-950/20 text-amber-700 dark:text-amber-400 text-[10px] font-bold px-2.5 py-1 rounded-md uppercase tracking-wide">Pending</span>
                                        @else
                                            <span class="bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 text-[10px] font-bold px-2.5 py-1 rounded-md uppercase tracking-wide">—</span>
                                        @endif
                                    </td>
                                    <td class="py-4 px-3">
