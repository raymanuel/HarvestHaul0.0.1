...
<div {{ $attributes->merge([
    ...
    <div class="pt-3 border-t border-slate-100 dark:border-slate-700/60 flex items-center justify-between mt-4">
        ...
        @else
            ...
            @if($linkText)
                <a href="{{ $href }}" class="text-gold-600 dark:text-gold-light font-bold text-xs hover:underline transition inline-flex items-center gap-1.5 group-hover:translate-x-1 duration-200">
                    {{ $linkText }} <span>→</span>
                </a>
            @endif
        @endif
