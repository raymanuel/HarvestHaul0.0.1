@props(['daPrices' => collect(), 'priceTrends' => collect(), 'latestDate' => null, 'scraperStatus' => null, 'compact' => true])
...
@php
    ...
    $sourceUrl = 'https://rfo12.da.gov.ph';
@endphp
...
<div class="bg-white dark:bg-slate-800/80 backdrop-blur border border-slate-200/60 dark:border-slate-700/60 rounded-3xl overflow-hidden">
    ...
    @else
        @if($compact)
            {{-- COMPACT MODE: Top picks across categories --}}
            @php
                $topPicks = $priceTrends->take(8);
            @endphp
            <div class="px-6 py-4 space-y-0 divide-y divide-slate-100 dark:divide-slate-700/30">
                @foreach($topPicks as $trend)
