<style>
    ...
    @keyframes neg-pulse-glow {
        0%, 100% { box-shadow: 0 0 24px rgba(22, 40, 60, 0.5), 0 0 80px rgba(22, 40, 60, 0.15); }
        50% { box-shadow: 0 0 40px rgba(22, 40, 60, 0.7), 0 0 100px rgba(22, 40, 60, 0.25); }
    }
    .dark .neg-glow {
    ...
    @keyframes neg-pulse-glow-dark {
        0%, 100% { box-shadow: 0 0 24px rgba(22, 40, 60, 0.4), 0 0 80px rgba(22, 40, 60, 0.15); }
        50% { box-shadow: 0 0 40px rgba(22, 40, 60, 0.6), 0 0 100px rgba(22, 40, 60, 0.2); }
    }
    .neg-badge-ping {
...
<div id="neg-widget" class="fixed bottom-6 right-6 z-50">
    ...
    <button id="neg-toggle"
        class="w-14 h-14 rounded-full bg-[#16283C] hover:bg-brand-dark dark:bg-[#16283C] dark:hover:bg-[#0E1620] text-white shadow-2xl shadow-[#16283C]/50 hover:shadow-[#16283C]/60 dark:shadow-[#16283C]/60 transition-all duration-200 hover:scale-110 active:scale-95 flex items-center justify-center cursor-pointer relative neg-glow ring-2 ring-white/30 dark:ring-white/20"
        aria-label="My Negotiations"
    >
    ...
    {{-- Popup --}}
    <div id="neg-popup"
        class="hidden absolute bottom-16 right-0 w-[calc(100vw-3rem)] max-w-96 max-h-[520px] bg-white dark:bg-slate-800/95 border border-slate-200/60 dark:border-[#16283C]/20 rounded-2xl shadow-2xl shadow-[#16283C]/10 dark:shadow-[#16283C]/20 overflow-hidden flex flex-col origin-bottom-right"
    >
        {{-- Header --}}
        <div class="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-[#16283C]/10 shrink-0 bg-gradient-to-r from-[#16283C]/5 to-transparent dark:from-[#16283C]/10 dark:to-transparent">
            <div>
                ...
                <p class="text-[10px] text-slate-500 dark:text-slate-400 font-medium">B2B crop price discussions</p>
            </div>
            <button id="neg-close" aria-label="Close negotiations panel" class="w-7 h-7 rounded-lg bg-slate-100 dark:bg-slate-700/60 hover:bg-slate-200 dark:hover:bg-slate-600/60 flex items-center justify-center text-slate-500 dark:text-slate-400 transition cursor-pointer">
...
(function() {
    ...
    function renderList(items) {
        ...
        items.forEach(item => {
            ...
            el.href = item.url;
            el.className = 'flex items-start gap-3 px-5 py-3.5 hover:bg-[#0E1620]/10/50 dark:hover:bg-[#0E1620]/10 transition group';
            el.innerHTML = `
                ...
                <div class="min-w-0 flex-1">
                    ...
                    <p class="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">${escapeHtml(item.counterpart_name)} ${volume}</p>
