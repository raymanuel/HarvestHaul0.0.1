        ...
        @if(Auth::user()->farmerProfile?->affiliation_type !== 'cooperative')
            <div class="flex gap-1 mb-8 bg-slate-100 dark:bg-slate-800/60 rounded-xl p-1 w-fit border border-slate-200/70 dark:border-slate-700/80">
                <a href="{{ route('farmer.proposals') }}"
                   class="px-5 py-2.5 rounded-lg text-sm font-bold transition {{ request()->routeIs('farmer.proposals') ? 'bg-white dark:bg-slate-700 text-slate-800 dark:text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300' }}">
                    Route Offers
                </a>
                <a href="{{ route('farmer.haul-requests') }}"
                   class="px-5 py-2.5 rounded-lg text-sm font-bold transition {{ request()->routeIs('farmer.haul-requests') ? 'bg-white dark:bg-slate-700 text-slate-800 dark:text-white shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300' }}">
                    Haul Requests
                </a>
            </div>
        @endif
                ...
                @foreach($haulRequests as $haulRequest)
                    <div class="bg-white dark:bg-slate-800 border border-slate-200/70 dark:border-slate-700/80 rounded-2xl shadow-sm p-6">
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <span class="text-[10px] font-bold uppercase tracking-wide px-3 py-1 rounded-lg border
                                    ...
                                    @foreach($haulRequest->intents as $intent)
                                            ...
                                            <div class="flex items-center gap-2 ml-4">
                                                ...
                                                @elseif($intent->status === 'declined')
                                                    <span class="text-xs font-bold text-[var(--color-error-text)] bg-[var(--color-error-bg)] px-3 py-1.5 rounded-lg border border-[var(--color-error-border)]">Declined</span>
                                                @endif
                                            </div>
                                        </div>
                                    @endforeach
