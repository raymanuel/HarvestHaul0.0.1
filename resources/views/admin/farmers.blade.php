...
<div class="w-full max-w-7xl mx-auto">
    ...
    <header class="pt-8 mb-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <span class="text-[10px] font-bold uppercase tracking-widest text-[var(--color-warning-text)] bg-[var(--color-warning-bg)] px-3 py-1.5 rounded-lg border border-[var(--color-warning-border)] self-start">{{ $farmers->count() }} Farmers</span>
        </div>
    </header>
        ...
        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-900/40 transition">
            <td class="px-4 py-3">
                <div class="flex items-center gap-3">
                    <div class="w-8 h-8 rounded-lg bg-gradient-to-tr from-[#16283C]/15 to-[#16283C]/10 dark:from-[#16283C]/10 dark:to-[#16283C]/5 border border-[#16283C]/20 dark:border-[#16283C]/15 flex items-center justify-center text-[10px] font-extrabold text-[#16283C] dark:text-[#D7BC7A] uppercase">{{ substr($farmer->name, 0, 2) }}</div>
                ...
                <div class="flex items-center gap-2">
                    @if(!$farmer->farmerProfile?->is_verified)
                        <form method="POST" action="{{ route('admin.farmers.verify', $farmer->id) }}">
                            @csrf
                            <button type="button" onclick="swalConfirm(this.closest('form'), {title: 'Approve Farmer?', text: 'Verify {{ addslashes($farmer->name) }} as a registered farmer?', confirmText: 'Yes, approve', icon: 'question', confirmColor: '#16283C'})" class="inline-flex items-center justify-center w-8 h-8 rounded-lg bg-[#16283C]/10 text-[#16283C] hover:bg-[#16283C]/15 dark:bg-[#16283C]/10 dark:hover:bg-[#16283C]/15 dark:text-[#D7BC7A] transition" title="Approve Farmer">
