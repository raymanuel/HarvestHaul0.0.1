<x-layout>
    <div class="w-full max-w-6xl mx-auto pb-12">
        ...
        <div class="mb-6 flex items-center justify-between">
            <h2 class="text-[10px] font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400">Crop Pricing Trends</h2>
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700/40">
                        @foreach($cropPricingTrends as $trend)
                            <tr class="hover:bg-slate-50/40 dark:hover:bg-slate-900/10 transition">
                                ...
                                <td class="px-5 py-3 text-right font-mono text-[#16283C] dark:text-[#D7BC7A] font-bold">₱{{ number_format($trend->avg_price, 2) }}</td>
        ...
        {{-- ═══════════════════════════════════════════ --}}
        <div class="mb-6 flex items-center justify-between">
            <h2 class="text-[10px] font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400">Logistics Efficiency</h2>
            <span class="w-32 h-px bg-slate-200 dark:bg-slate-700/80"></span>
        </div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
            ...
            <div class="bg-white dark:bg-slate-800 border border-slate-200/70 dark:border-slate-700/80 rounded-2xl p-5 shadow-sm">
                ...
                <p class="text-2xl font-bold text-[var(--color-warning-text)]">₱{{ number_format($totalFuelCost, 0) }}</p>
            ...
            <div class="bg-white dark:bg-slate-800 border border-slate-200/70 dark:border-slate-700/80 rounded-2xl p-5 shadow-sm">
                ...
                <p class="text-2xl font-bold text-[#16283C] dark:text-[#D7BC7A]">{{ $avgKpl > 0 ? $avgKpl : '—' }}</p>
            </div>
        </div>
        ...
        {{-- ═══════════════════════════════════════════ --}}
        <div class="mb-6 flex items-center justify-between">
            <h2 class="text-[10px] font-bold uppercase tracking-widest text-slate-500 dark:text-slate-400">Baseline Price Management</h2>
            <span class="w-32 h-px bg-slate-200 dark:bg-slate-700/80"></span>
        </div>
                        ...
                        <tr class="hover:bg-slate-50/40 dark:hover:bg-slate-900/10 transition">
                            ...
                            <td class="px-5 py-3 text-right font-mono">
                                @if($crop->baseline_price_per_kg)
                                    <span class="text-[#16283C] dark:text-[#D7BC7A] font-bold">₱{{ number_format($crop->baseline_price_per_kg, 2) }}/kg</span>
                                @else
                            ...
                            <td class="px-5 py-3 text-right">
                                <form method="POST" action="{{ route('admin.baseline-price', $crop) }}" class="inline-flex items-center gap-2">
                                    ...
                                    <input type="number" name="baseline_price_per_kg" step="0.01" min="0.01"
                                           ...
                                           placeholder="₱/kg"
                                           class="w-24 text-right text-xs border border-slate-200 dark:border-slate-600 rounded-lg px-2 py-1.5 bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 focus:ring-2 focus:ring-[#16283C]/30 focus:border-[#16283C]">
                                    <button type="submit"
                                            class="text-[10px] font-bold uppercase px-2.5 py-1.5 bg-[#16283C]/10 dark:bg-[#16283C]/10 text-[#16283C] dark:text-[#D7BC7A] border border-[#16283C]/20 dark:border-[#16283C]/20 rounded-lg hover:bg-[#16283C]/15 hover:bg-[#16283C]/20 transition cursor-pointer">
                                        Save
                                    </button>
                                </form>
                            </td>
