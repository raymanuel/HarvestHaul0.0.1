...
trait Notifiable
{
    ...
    protected static function notifyNegotiationStarted(int $farmerId, string $buyerName, int $harvestId, string $cropType): void
    {
        static::sendNotification(
            $farmerId,
            'New B2B Negotiation',
            "{$buyerName} is interested in your product #{$harvestId} ({$cropType}).",
            route('negotiations.room', $harvestId),
            'negotiation_started',
            'negotiation'
        );
    }
    ...
    protected static function notifyHarvestPosted(int $userId, int $harvestId, string $cropType, ?int $coopLogisticsUserId): void
    {
        ...
        if ($coopLogisticsUserId) {
            static::sendNotification(
                ...
                "A cooperative member posted crop #{$harvestId} ({$cropType}). Ready for route planning.",
