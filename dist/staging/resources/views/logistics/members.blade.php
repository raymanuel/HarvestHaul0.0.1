        ...
        <div class="space-y-8">
            ...
            <div>
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <span class="inline-flex items-center justify-center min-w-[22px] h-[22px] px-2 bg-amber-50 text-amber-700 dark:bg-amber-950/20 dark:text-amber-400 rounded-md text-[10px] font-extrabold">{{ $pendingRequests->count() }}</span>
                </div>
                            ...
                            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-900/40 transition">
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-3">
                                        <div class="w-8 h-8 rounded-md bg-gradient-to-tr from-slate-100 to-slate-50 dark:from-slate-700 dark:to-slate-800 border border-slate-200 dark:border-slate-700 flex items-center justify-center text-[10px] font-extrabold text-slate-600 dark:text-slate-300 uppercase shrink-0">{{ substr($req->user->name, 0, 2) }}</div>
                                ...
                                <td class="px-4 py-3 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <form action="{{ route('logistics.members.approve', $req->id) }}" method="POST" class="inline" id="approve-form-{{ $req->id }}">
                                            @csrf
                                            <button type="button" onclick="swalConfirm(document.getElementById('approve-form-{{ $req->id }}'), {title: 'Approve Member?', text: 'Approve {{ addslashes($req->user->name) }} as a cooperative member?', confirmText: 'Yes, approve', icon: 'question', confirmColor: '#16283C'})" class="inline-flex items-center justify-center gap-1.5 px-3.5 py-2 bg-[#16283C]/10 text-[#16283C] hover:bg-[#16283C]/15 dark:bg-[#16283C]/10 dark:hover:bg-[#16283C]/15 dark:text-[#D7BC7A] rounded-xl text-xs font-bold transition cursor-pointer">
            ...
            <div>
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <span class="inline-flex items-center justify-center min-w-[22px] h-[22px] px-2 bg-[#16283C]/10 text-[#16283C] dark:bg-[#16283C]/10 dark:text-[#D7BC7A] rounded-md text-[10px] font-extrabold">{{ $members->count() }}</span>
                </div>
                    ...
                    <x-data-table>
                        ...
                        @foreach($members as $member)
                            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-900/40 transition">
                                ...
                                <td class="px-4 py-3 text-right">
                                    <form action="{{ route('logistics.members.remove', $member->id) }}" method="POST" class="inline" id="remove-form-{{ $member->id }}">
                                        @csrf
                                        <button type="button" onclick="swalConfirm(document.getElementById('remove-form-{{ $member->id }}'), {title: 'Remove Member?', text: 'Remove {{ addslashes($member->user->name) }} from the cooperative? They will return to independent farming.', confirmText: 'Yes, remove', icon: 'warning', confirmColor: '#ef4444'})" class="inline-flex items-center justify-center gap-1.5 px-3.5 py-2 bg-red-50 text-red-600 hover:bg-red-100 dark:bg-red-950/20 dark:hover:bg-red-950/40 dark:text-red-400 rounded-xl text-xs font-bold transition cursor-pointer">
