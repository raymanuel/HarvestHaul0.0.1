            ...
            <form method="POST" action="{{ route('farmer.documents.store') }}" enctype="multipart/form-data" class="space-y-4">
                ...
                <div class="form-group space-y-1.5">
                    ...
                    <input type="file" name="document_file" required accept=".jpg,.jpeg,.png,.pdf"
                        class="py-2 px-4 w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#16283C]/20 focus:border-[#16283C] transition text-sm text-slate-700 dark:text-slate-200 file:mr-4 file:py-1.5 file:px-3 file:rounded-md file:border-0 file:text-xs file:font-bold file:bg-slate-50 dark:file:bg-slate-800 file:text-slate-700 dark:file:text-slate-200 hover:file:bg-slate-100 dark:hover:file:bg-slate-700">
                    ...
                    @foreach($documents as $doc)
                        ...
                        <div class="border rounded-xl p-4 flex items-center justify-between flex-wrap gap-3 {{ $statusStyle }}">
                            ...
                            <div class="flex items-center gap-3.5">
                                <span class="text-[10px] font-bold px-2.5 py-1 rounded-md uppercase tracking-wide border {{ $badgeStyle }}">
                                    {{ $doc->status }}
                                </span>
                                ...
                                @if($doc->status !== 'approved')
                                    <form method="POST" action="{{ route('farmer.documents.destroy', $doc->id) }}">
                                        ...
                                        <button type="button"
                                            ...
                                            class="inline-flex items-center justify-center w-7 h-7 rounded-xl bg-rose-50 text-rose-600 hover:bg-rose-100 dark:bg-rose-950/20 dark:hover:bg-rose-950/40 dark:text-rose-400 transition cursor-pointer"
