<x-layout>
    <div class="w-full max-w-7xl mx-auto">
        ...
        <header class="pt-8 mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-slate-900 dark:text-white tracking-tight heading-font">Haul Deals</h1>
            </div>
        </header>
        ...
        @if($intents->isEmpty())
            <div class="bg-white dark:bg-slate-800 border border-slate-200/70 dark:border-slate-700/80 rounded-2xl p-12 text-center shadow-sm">
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-1 leading-relaxed">Haul intents from farmers will appear here once a negotiation starts.</p>
            </div>
        @else
            <div class="space-y-4">
                        ...
                        <div class="flex items-center gap-2">
                            ...
                            @else
                                <a href="{{ route('haul-negotiations.room', $intent->id) }}"
                                   class="inline-flex items-center gap-1.5 px-4 py-2 bg-[#16283C] hover:bg-[#0E1620] text-white dark:bg-[#D7BC7A] dark:hover:bg-[#BFA05A] dark:text-[#17202B] text-xs font-bold rounded-xl transition cursor-pointer">
                                    Open Chat
                                </a>
                            @endif
