...
<div class="w-full">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            @if (Auth::user()->farmerProfile?->is_verified)
                <a href="{{ route('harvests.create') }}"
                    class="bg-[#16283C] text-white px-6 py-3 rounded-xl font-semibold hover:bg-[#0E1620] dark:bg-[#16283C] dark:hover:bg-[#0E1620] transition shadow-md text-sm self-start sm:self-center whitespace-nowrap">
                    + Post New Harvest
                </a>
            @else
                <div class="flex items-center gap-2 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl px-5 py-3 self-start sm:self-center">
                    ...
                    <span class="text-xs bg-[var(--color-warning-bg)] text-[var(--color-warning-text)] border border-[var(--color-warning-border)] font-bold px-2.5 py-1 rounded-lg uppercase tracking-wider">
                        Pending Approval
                    </span>
                </div>
            @endif
    ...
    <div class="flex items-center gap-2 mb-6 border-b border-slate-200/70 dark:border-slate-700/70">
        <a href="{{ route('harvests.index', ['tab' => 'active']) }}"
            class="px-4 py-2.5 text-sm font-semibold transition border-b-2 -mb-px {{ ($tab ?? 'active') === 'active' ? 'border-[#16283C] text-[#16283C] dark:border-[#16283C] dark:text-[#D7BC7A]' : 'border-transparent text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300' }}">
            Active
        </a>
        <a href="{{ route('harvests.index', ['tab' => 'history']) }}"
            class="px-4 py-2.5 text-sm font-semibold transition border-b-2 -mb-px {{ ($tab ?? 'active') === 'history' ? 'border-[#16283C] text-[#16283C] dark:border-[#16283C] dark:text-[#D7BC7A]' : 'border-transparent text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300' }}">
            History
        </a>
    </div>
    ...
    @if($harvests->isEmpty())
        <div class="bg-slate-50 dark:bg-slate-900/40 border border-dashed border-slate-300 dark:border-slate-700/80 rounded-xl p-16 text-center">
            ...
            @if (Auth::user()->farmerProfile?->is_verified && ($tab ?? 'active') === 'active')
                <a href="{{ route('harvests.create') }}"
                    class="bg-[#16283C] text-white px-6 py-3 rounded-xl font-semibold hover:bg-[#0E1620] transition shadow-md text-sm inline-block">
                    ...
                    <tbody class="divide-y divide-slate-50 dark:divide-slate-700/40">
                        ...
                        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-900/40 transition">
                            ...
                            <td class="px-6 py-4">
                                @php
                                    $statusClasses = [
                                        'active'         => 'bg-[#16283C]/10 text-[#16283C] border-[#16283C]/10 dark:text-[#D7BC7A]',
                                ...
                                @endphp
                                <span class="{{ $statusClass }} text-[10px] font-bold px-2.5 py-1 rounded-lg uppercase tracking-wider border">{{ $statusLabel }}</span>
                            </td>
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    @if($canMarkSold)
                                        <form method="POST" action="{{ route('harvests.mark-as-sold', $harvest->id) }}" class="inline">
                                            ...
                                            <button type="button"
                                                ...
                                                class="inline-flex items-center gap-1 text-[10px] font-bold text-[#16283C] dark:text-[#D7BC7A] bg-[#16283C]/10 hover:bg-[#16283C]/15 dark:bg-[#16283C]/10 dark:hover:bg-[#16283C]/15 px-2.5 py-1.5 rounded-lg transition active:scale-[0.97]"
                                                title="Mark as Sold">
                                    ...
                                    @if($canCreateHaulRequest)
                                        <button type="button"
                                            ...
                                            class="inline-flex items-center gap-1 text-[10px] font-bold text-purple-700 dark:text-purple-400 bg-purple-50 hover:bg-purple-100 dark:bg-purple-950/20 dark:hover:bg-purple-950/40 px-2.5 py-1.5 rounded-lg transition active:scale-[0.97]"
                                            title="Request Haul">
