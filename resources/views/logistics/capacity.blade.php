    ...
    <div class="w-full max-w-4xl mx-auto pb-12">
        ...
        @php
            $surplusColor = match(true) {
                $surplusShortage > 0  => 'text-[#16283C] dark:text-[#D7BC7A]',
                $surplusShortage == 0 => 'text-amber-700 dark:text-amber-400',
                ...
                @foreach($recentJobs as $job)
                    @php
                        $statusColor = match($job['status']) {
                            'completed'   => 'text-[#16283C] dark:text-[#D7BC7A] bg-[#16283C]/10 dark:bg-[#16283C]/10',
                            'in_progress' => 'text-[#0E1620] dark:text-[#E9EEF4] bg-[#0E1620]/10 dark:bg-[#0E1620]/10',
                            'confirmed'   => 'text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/20',
        ...
        {{-- Disclaimer --}}
        <div class="bg-slate-50 dark:bg-slate-800/40 border border-slate-200/60 dark:border-slate-700/60 rounded-xl px-5 py-4 text-xs text-slate-500 dark:text-slate-400 font-semibold">
            <svg class="w-4 h-4 inline -mt-0.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg> Estimate uses average kg per completed job. Actual requirements depend on route distance, pickup density, and cargo type.
        </div>
    </div>
</x-layout>
