...
<nav class="flex-1 px-3 py-6 overflow-y-auto custom-scroll space-y-1.5" aria-label="Sidebar navigation">
    ...
    <a href="/dashboard" data-tooltip="Dashboard" aria-label="Dashboard" @if(request()->is('dashboard'))aria-current="page"@endif class="nav-link flex items-center px-4 py-2.5 rounded-xl text-sm font-semibold transition {{ request()->is('dashboard') ? 'nav-active' : '' }}">
        <span class="nav-letter shrink-0 w-8 h-8 rounded-md bg-slate-900/5 text-slate-500 dark:bg-white/10 dark:text-white/70 text-xs font-bold flex items-center justify-center uppercase">D</span>
        <span class="nav-label">Dashboard</span>
    </a>

    @if($sectionLabel)
        <p class="section-label mt-8 text-[10px] font-bold text-slate-400 dark:text-white/60 uppercase tracking-widest px-4">{{ $sectionLabel }}</p>
    @endif
        ...
        @if($hasChildren)
            ...
            <button type="button" data-submenu-toggle data-tooltip="{{ $item['tooltip'] }}" aria-label="{{ $item['label'] }}" aria-expanded="{{ $isSubmenuActive ? 'true' : 'false' }}" class="nav-link w-full flex items-center justify-between px-4 py-2.5 rounded-xl text-sm font-semibold transition select-none {{ $isSubmenuActive ? ' nav-active' : '' }}">
                    <div class="flex items-center gap-3">
                        <span class="nav-letter shrink-0 w-8 h-8 rounded-md bg-slate-900/5 text-slate-500 dark:bg-white/10 dark:text-white/70 text-xs font-bold flex items-center justify-center uppercase">{{ $item['letter'] }}</span>
                ...
                <div data-submenu-panel role="group" class="submenu-panel {{ $isSubmenuActive ? 'submenu-open' : '' }} pl-4">
                        ...
                        @if($childCondition === 'farmer_non_cooperative')
                            @if(Auth::user()->farmerProfile?->affiliation_type !== 'cooperative')
                                <a href="{{ route($child['route_url']) }}" data-tooltip="{{ $child['tooltip'] }}" aria-label="{{ $child['label'] }}" @if($childIsActive)aria-current="page"@endif class="nav-link flex items-center px-4 py-2.5 rounded-xl text-sm font-semibold transition {{ $childActiveClass }}">
                                    <span class="nav-letter shrink-0 w-8 h-8 rounded-md bg-slate-900/5 text-slate-500 dark:bg-white/10 dark:text-white/70 text-xs font-bold flex items-center justify-center uppercase">{{ $child['letter'] }}</span>
