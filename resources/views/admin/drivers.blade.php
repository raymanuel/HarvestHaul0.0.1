...
<div class="w-full max-w-7xl mx-auto">
    ...
    <header class="pt-8 mb-8">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-slate-800 dark:text-white heading-font tracking-tight">Driver Accounts</h1>
            </div>
            <span class="text-[10px] font-bold uppercase tracking-widest text-[#0E1620] dark:text-[#E9EEF4] bg-[#0E1620]/10 dark:bg-[#0E1620]/10 px-3 py-1.5 rounded-md border border-[#0E1620]/10 dark:border-[#0E1620]/20 self-start">{{ $drivers->count() }} Drivers</span>
        </div>
    </header>
        ...
        <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-900/40 transition">
            <td class="px-4 py-3">
                <div class="flex items-center gap-3">
                    <div class="w-8 h-8 rounded-md bg-[#0E1620]/10 dark:bg-[#0E1620]/10 border border-[#0E1620]/20 dark:border-[#0E1620]/15 flex items-center justify-center text-[10px] font-extrabold text-[#0E1620] dark:text-[#E9EEF4] uppercase">{{ substr($driver->name, 0, 2) }}</div>
            ...
            <td class="px-4 py-3">
                ...
                @else
                    <span class="text-slate-500 dark:text-slate-400 text-[10px] font-bold">—</span>
                @endif
            </td>
            <td class="px-4 py-3">
                <x-badge :status="$driver->status === 'active' ? 'active' : 'inactive'" />
            </td>
            <td class="px-4 py-3 text-slate-500 dark:text-slate-400 text-xs font-semibold">{{ $driver->created_at->format('M d, Y') }}</td>
            <td class="px-4 py-3">
                <div class="flex items-center gap-2">
                    @if($driver->driverProfile?->id_photo_path && !$driver->driverProfile?->identity_verified)
                        <form method="POST" action="{{ route('admin.drivers.verify-identity', $driver->id) }}" class="inline">
                            @csrf
                            <button type="button" onclick="swalConfirm(this.closest('form'), {title: 'Verify Identity?', text: 'Verify identity for {{ addslashes($driver->name) }}?', confirmText: 'Yes, verify', icon: 'success', confirmColor: '#16283C'})" class="inline-flex items-center justify-center w-8 h-8 rounded-xl bg-[#16283C]/10 text-[#16283C] hover:bg-[#16283C]/15 dark:bg-[#16283C]/10 dark:hover:bg-[#16283C]/15 dark:text-[#D7BC7A] transition" title="Verify Identity">
                        ...
                        <form method="POST" action="{{ route('admin.drivers.reject-identity', $driver->id) }}" class="inline">
                            @csrf
                            <button type="button" onclick="swalConfirm(this.closest('form'), {title: 'Reject Identity?', text: 'Reject identity for {{ addslashes($driver->name) }}?', confirmText: 'Yes, reject', icon: 'warning', confirmColor: '#ef4444'})" class="inline-flex items-center justify-center w-8 h-8 rounded-xl bg-red-50 text-red-600 hover:bg-red-100 dark:bg-red-950/20 dark:hover:bg-red-950/40 dark:text-red-400 transition" title="Reject Identity">
                    ...
                    <form method="POST" action="{{ route('admin.users.status', $driver->id) }}">
                        ...
                        @if($driver->status === 'active')
                            <button type="button" onclick="swalConfirm(this.closest('form'), {title: 'Archive Driver?', text: 'Archive {{ addslashes($driver->name) }}?', confirmText: 'Yes, archive', icon: 'warning', confirmColor: '#f59e0b'})" class="inline-flex items-center justify-center w-8 h-8 rounded-xl bg-amber-50 text-amber-700 hover:bg-amber-100 dark:bg-amber-950/20 dark:hover:bg-amber-950/40 dark:text-amber-400 transition" title="Archive Driver">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" />
                                </svg>
                            </button>
                        @else
                            <button type="button" onclick="swalConfirm(this.closest('form'), {title: 'Reactivate Driver?', text: 'Reactivate {{ addslashes($driver->name) }}?', confirmText: 'Yes, reactivate', icon: 'question', confirmColor: '#16283C'})" class="inline-flex items-center justify-center w-8 h-8 rounded-xl bg-[#16283C]/10 text-[#16283C] hover:bg-[#16283C]/15 dark:bg-[#16283C]/10 dark:hover:bg-[#16283C]/15 dark:text-[#D7BC7A] transition" title="Reactivate Driver">
                                <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
