...
class Harvest extends Model
{
    ...
    protected $casts = [
        ...
        'estimated_volume_cubic_m' => 'decimal:2',
        'latitude'                 => 'decimal:8',
        'longitude'                => 'decimal:8',
    ...
    // ─────────────────────────────────────────────────────────
    // COOP LEAVE / REMOVE HELPERS
    // ─────────────────────────────────────────────────────────

    /** Open posts whose saved drop-off points at the given coordinates. */
    public function scopeOpenWithDestinationAt($query, float $lat, float $lng)
    {
        return $query
            ->whereIn('status', [HarvestStatus::ACTIVE, HarvestStatus::PARTIALLY_SOLD])
            ->whereNotNull('destination_latitude')
            ->whereNotNull('destination_longitude')
            ->whereRaw('ABS(destination_latitude - ?) < 0.0005', [$lat])
            ->whereRaw('ABS(destination_longitude - ?) < 0.0005', [$lng]);
    }

    /**
     * Clear the drop-off snapshot on a farmer's open posts whose saved
     * destination is at the given coordinates.  Returns the number of
     * harvests that were actually updated.
     */
    public static function resetHubDestinationForUser(int $userId, float $lat, float $lng): int
    {
        $ids = self::where('user_id', $userId)
            ->openWithDestinationAt($lat, $lng)
            ->pluck('id');
